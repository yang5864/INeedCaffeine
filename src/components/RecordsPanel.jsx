...
import { DAILY_MAX_MG } from "../utils/caffeine.js";
...
export default function RecordsPanel({ logs, now, dailyLimitMg = DAILY_MAX_MG, halfLifeHours }) {
  const days = buildDailyRecords(logs, now, halfLifeHours);
  const entries = buildRecordEntries(logs, now, halfLifeHours);
  const summary = buildRecordsSummary(logs, now, dailyLimitMg, halfLifeHours);
  ...
  return (
    <motion.section
      ...
      className="mx-auto grid max-h-full w-full max-w-7xl gap-4 rounded-[30px] border border-white/35 bg-white/20 p-5 text-white shadow-glass backdrop-blur-2xl md:p-6 lg:grid-cols-[0.88fr_1.12fr]"
    >
      <div className="min-w-0">
        <div className="flex items-center gap-3">
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-white/20 shadow-soft">
            <History className="h-5 w-5 text-white" />
          </span>
          <div className="min-w-0">
            <p className="text-xs font-bold uppercase tracking-[0.22em] text-cyan-100">Records Arcade</p>
            <h2 className="truncate text-3xl font-black tracking-wide drop-shadow-sm">카페인 기록</h2>
          </div>
        </div>
        ...
        <div className="mt-4 max-h-[230px] space-y-2.5 overflow-y-auto pr-1">
          {days.length === 0 ? (
            <EmptyState />
          ) : (
      ...
      <div className="min-w-0 rounded-[26px] border border-white/24 bg-white/12 p-4 shadow-soft md:p-5">
...
function SummaryTile({ icon: Icon, label, value, sub }) {
  return (
    <div className="min-w-0 rounded-[22px] border border-white/24 bg-white/14 p-3.5 shadow-soft">
      <div className="flex items-center gap-2 text-white/75">
        <Icon className="h-4 w-4 shrink-0" />
        <span className="truncate text-sm font-semibold">{label}</span>
      </div>
      <div className="mt-1.5 flex items-end gap-2">
        <strong className="truncate text-lg font-black text-white">{value}</strong>
...
function DaySummary({ day, dailyLimitMg }) {
  return (
    <article className="rounded-[22px] border border-white/24 bg-white/12 p-3.5 shadow-soft">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <h3 className="truncate text-base font-black">{day.dayLabel}</h3>
          <p className="mt-0.5 text-sm font-bold text-white/70">{day.count}번 섭취</p>
        </div>
        <strong className="shrink-0 text-base font-black">{mg(day.totalMg)}</strong>
      </div>
      ...
      <p className="mt-1.5 text-sm font-bold text-white/68">현재 남은 추정량 {mg(day.currentMg)}</p>
    </article>
  );
}
...
function RecordRow({ entry }) {
  return (
    <article className="grid grid-cols-[40px_1fr_auto] items-center gap-3 rounded-[20px] border border-white/18 bg-white/12 p-2.5">
      <span className="grid h-10 w-10 place-items-center rounded-full bg-white/22">
        <Coffee className="h-5 w-5 fill-white/70 text-white" />
      </span>
      <div className="min-w-0">
        <p className="truncate text-sm font-black">{mg(entry.amountMg)} 섭취</p>
